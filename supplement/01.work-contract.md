...
The operational contract may change as the project evolves. This supplement is a fixed publication snapshot of the contract maintained by the User.
...
No article, preprint, DOI, arXiv identifier, or article URL may enter a manuscript, report, notebook, or public documentation until the following process is followed:

1. A project participant proposes a new resource by opening a record in a citation manager, citation ledger or the Collaboration log. The new item record is marked "provisional."

2. The participant must confirm the existence of the record by accessing the source and then using that external source to record title, authors, publication year, and any persistent identifiers. The date of access and relevant location(s) of the new resource are included in the new record.  Publications should be cited with  DOIs (or other access points such as  URLs if no DOIs) and are recorded in the corresponding entry.  
3. The citation source (DOI, URL) must be checked to see if it corresponds with the log entry by at least one participant before acceptance into the project. Any updates or changes are recorded by the reviewers. 
4. The reviewers can then mark  the record as accepted. 
5. An unresolved, broken, mismatched, or unvalidated citation will block merge and release. 
...
## S2.5 RULE FOUR:  Durable records are generated consistently and constantly.
...
## S2.6  Roles are flexible. 

1. All participants must be available for complementary and reversible roles
2. No platform is presumed to be globally superior. Roles are assigned by task.
