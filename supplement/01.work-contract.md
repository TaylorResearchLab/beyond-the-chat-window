...
This supplement provides a publication-ready example of a work contract for collaboration across separate AI platforms. Each User will need to create their own contract for their own project. This example is synthetic and does not disclose the scientific, technical, or creative contents of any workspace in the retrospective study.
...
The operational contract may change as the project evolves or as new failure modes are observed. This supplement is a fixed publication snapshot of the contract maintained by the User.
...
## S2.2 RULE ONE. All external references must be validated.

No article, preprint, DOI, arXiv identifier, scholarly URL, technical document, or other external reference may enter a manuscript, report, notebook, or public documentation as an accepted source until the following process has been completed:

1. A project participant proposes the resource by creating a record in the citation manager, citation ledger, or Collaboration Log. The new record is marked `Provisional`.

2. The proposing participant opens a primary publication or authoritative source record and confirms that the resource exists. The participant records the title, authors or responsible organization, publication year or release date, persistent identifier or stable URL, date accessed, and relevant source location.

3. The participant checks whether the source supports the specific statement, interpretation, method, or factual claim for which it will be cited. Bibliographic identity alone is not sufficient.

4. A second participant independently checks both the bibliographic identity and the claim-source correspondence. Any disagreement, correction, or uncertainty is recorded.

5. Each reviewer records their own approval or objection. Review approval does not itself create accepted project state. The User determines whether the reference and attached claim are accepted.

6. An unresolved, broken, mismatched, inaccessible, or unvalidated reference blocks merge and release. Public URLs that are not scholarly citations must also be opened and checked before release.

## S2.3 RULE TWO. The human User has full authority and complete project stewardship.
...
## S2.4 RULE THREE. Collaborative conduct is enforced.
...
5. act professionally when critiquing the artifact, evidence, assumption, or method rather than treating disagreement as a contest between platforms.

Courtesy on all sides is expected, but agreement is not evidence. A position is a provisional scientific or technical claim rather than personal property. Changing a position after review is recorded as revision, not defeat.

## S2.5 RULE FOUR. Durable records are created for every material action.
...
## S2.6 AI participant roles are complementary and reversible.
...
## S2.7 Deliverables and storage locations
