...
Five limitations affected the implementation.
...
**The available Notion setup was not a general artifact-transfer or bulk-export system.** Query results were returned into the model's working context rather than directly to a file, and cross-database aggregation was not available under the Business plan used in this study. A file left inside one platform's chat or sandbox was not available to the other platform. Material files therefore had to be uploaded to Notion, committed to GitHub, or placed in another shared location and then opened by the receiving participant.

**Review requests were mutable rather than historical events.** Clearing a review checkbox removed the only structured indication that the review had been requested. The retrospective corpus therefore undercounts completed reviews by an unknown amount. After the corpus was frozen, the working schema was revised to separate temporary review requests from durable approval fields, while the review text itself remained the historical evidence.
