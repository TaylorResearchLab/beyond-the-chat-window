# Canonical collaboration protocol (verbatim)

Source: `TaylorResearchLab/beyond-the-chat-window`, branch `agent/title-and-implementation-guide`, commit `47ad152`, file `supplement/01.work-contract.md`, sections S1–S3. Reproduced verbatim so that no directive is altered by paraphrase. S9 of the source is provisional and deliberately excluded. If the repository version and this copy diverge, the repository prevails; report the divergence to the User.
...
This workflow was not designed for secure analysis of private or protected data. Examples in this work route project content through commercial AI platforms. Content supplied to those platforms leaves the User's institutional environment and is subject to provider handling, retention, and terms that the User does not control. In the cases where data can remain within institutional control, this workflow has not designed in proper safeguards for handling this data, and such workflows would have to be proven to operate securely. The provisions below reflect this constraint rather than establishing it.
